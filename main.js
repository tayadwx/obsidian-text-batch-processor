...
var AiPromptModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", {
      cls: "text-batch-muted",
      text: "\u8FD9\u662F\u7528\u4E8E\u8BA9\u7F51\u9875\u7248 Chat AI \u751F\u6210\u300C\u53EF\u5BFC\u5165\u914D\u7F6E JSON\u300D\u7684\u63D0\u793A\u8BCD\u3002\u53EF\u624B\u52A8\u4FEE\u6539\u5E76\u4FDD\u5B58\u4EE5\u4FBF\u957F\u671F\u4F7F\u7528\uFF1B\u4E5F\u53EF\u4E00\u952E\u6062\u590D\u4E3A\u5185\u7F6E\u9ED8\u8BA4\u63D0\u793A\u8BCD\uFF0C\u6216\u4ECE\u7F51\u5740\u62C9\u53D6\uFF08\u7559\u7A7A\u5219\u7528\u9ED8\u8BA4\u7F51\u5740\uFF09\u3002"
    });
    const fetchRow = contentEl.createEl("div", { cls: "text-batch-aiprompt-fetch" });
    const urlInput = new import_obsidian5.TextComponent(fetchRow);
    urlInput.inputEl.classList.add("text-batch-aiprompt-url");
    urlInput.setPlaceholder("\u7559\u7A7A\u5219\u4F7F\u7528\u9ED8\u8BA4\u7F51\u5740");
    fetchRow.createEl("button", { text: "\u4ECE\u7F51\u5740\u83B7\u53D6", cls: "mod-cta" }).addEventListener("click", async () => {
      const raw = urlInput.getValue().trim();
      const target = raw || DEFAULT_AI_PROMPT_URL;
      try {
        const resp = await (0, import_obsidian5.requestUrl)({ url: target });
        if (resp.status !== 200) {
          new import_obsidian5.Notice(`\u4ECE\u7F51\u5740\u83B7\u53D6\u5931\u8D25\uFF1AHTTP ${resp.status}`);
          return;
        }
        if (!resp.text || !resp.text.trim()) {
          new import_obsidian5.Notice("\u4ECE\u7F51\u5740\u83B7\u53D6\u7684\u5185\u5BB9\u4E3A\u7A7A");
          return;
        }
        this.ta.setValue(resp.text);
        new import_obsidian5.Notice("\u5DF2\u62C9\u53D6\u63D0\u793A\u8BCD\uFF0C\u8BF7\u68C0\u67E5\u540E\u70B9\u51FB\u300C\u4FDD\u5B58\u300D");
      } catch (e) {
        new import_obsidian5.Notice("\u4ECE\u7F51\u5740\u83B7\u53D6\u5931\u8D25\uFF1A" + (e?.message || String(e)));
      }
    });
    contentEl.createEl("p", {
